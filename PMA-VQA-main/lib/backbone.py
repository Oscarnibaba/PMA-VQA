...
class MMBasicLayer(nn.Module):
    def __init__(self,
        ...
        self.fusion = SAAM(dim,  # both the visual input and for combining, num of channels
                           dim,  # v_in
                           768,  # l_in
                           dim,  # key
                           dim,  # value
...
class SAAM(nn.Module):
    ...
    def forward(self, x, l, l_mask):
        ...
        lang_m = self.image_lang_att(x, l, l_mask)  # (B, H*W, dim)
        lang_m = lang_m.permute(0, 2, 1)  # (B, dim, H*W)


        spatial_weights = self.spatial_gate(lang_m)
        mm = spatial_weights * vis + (1 - spatial_weights) * lang_m
